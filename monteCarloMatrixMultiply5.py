...
def monteCarloMatrixMultiply2(A,B):
    ...
    return AB
omega = 2 + random()
def testPoly(x,a,b,c,d,e):
    return a*x**(3*e) + b*x**(2*e) + c*x**e + d
setparams = [5,45,5]
...
params, params_covariance = optimize.curve_fit(
    ...
    p0=[1, 1, 1, 1, 2],
    bounds = ([-np.inf,-np.inf,-np.inf,-np.inf,1.5],
    [np.inf,np.inf,np.inf,np.inf,2.1]))
print(
    '(',params[0],')x^'+str(3*params[4]),' + (',
    params[1],')x^'+str(2*params[4]),' + (',
    params[2],')x^'+str(params[4])+' +',params[3]
    )